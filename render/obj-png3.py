...
angles = 16
img_size = 256         # увеличь для качества (128/256/512 и т.д.)
elevation = math.radians(35.264)   # угол для изометрии (35.264 — классика)

# --- Вспомогательная функция: камера всегда смотрит на target ---
def look_at(obj_camera, target):
    direction = Vector(target) - obj_camera.location
    rot_quat = direction.to_track_quat('-Z', 'Y')
    obj_camera.rotation_euler = rot_quat.to_euler()

# --- Получить центр и максимальный размер объекта ---
...
for obj_file in obj_files:
    ...
    # cam.data.shift_x = 0.0   # если надо — настрой, но лучше не трогать пока
    # cam.data.shift_y = 0.0

    # --- Радиус (на сколько камера отъезжает, чтобы весь объект влез) ---
    radius = max(4, max_dim * 1.2)   # Можешь изменить коэффициент под себя
